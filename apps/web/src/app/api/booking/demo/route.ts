...
export async function POST(request: Request) {
  try {
    ...
    // meeting time = now (so the "start call" button appears on the dashboard)
    ...
    // create call + pipeline run (so meeting/build flow works)
    ...
    // create tracker with password auth
